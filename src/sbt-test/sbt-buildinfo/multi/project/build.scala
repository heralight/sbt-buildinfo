...
object Build extends sbt.Build {
  ...
  lazy val appSettings = buildSettings ++ liftInfoSettings ++ Seq(
    name := "sbt-liftinfo-example-app",
    sourceGenerators in Compile <+= liftInfo,
    liftInfoKeys := Seq(name,
                         projectID in "root",
                         version,
                         LiftInfoKey.map(homepage) { case (n, opt) => n -> opt.get },
                         scalaVersion),
    ...
    check <<= (sourceManaged in Compile) map { (dir) =>
      ...
      lines match {
        case """package hello""" ::
             ...
             """  /** The value is "sbt-liftinfo-example-app". */""" ::
             """  val name = "sbt-liftinfo-example-app"""" ::
